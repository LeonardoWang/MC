...
class MnistNetwork(object):
    ...
    def build_network(self):
        ...
        with tf.name_scope('conv1'):
            ...
            """@nni.compression.pruning(w_conv1, prune_ratio, prune_dimension)"""
            #Configure().add_pruner_config(w_conv1 , 0.5, None, 'TensorflowLevelParameterPruner')
            Compression().param_mask(w_conv1, 0.5, None, 'TensorflowLevelParameterPruner')
            h_conv1 = tf.nn.relu(conv2d(x_image, w_conv1) + b_conv1)


            '''
            w_conv1 = func(w_conv1):
                mask = get_variable()
                indice = min_norm(w_conv1)
                mask = 
                return w_conv1 * mask
            '''
...
def main(params):
    ...
    with tf.Session() as sess:
        ...
        level = {}
        level['conv1/Variable:0'] = 0.5
        '''
        scheduler = CompressionScheduler(sess)
        pruner = TensorflowLevelParameterPruner(level)
        policy = TensorflowPruningPolicy(pruner,level)
        scheduler.addPolicy(policy,None,0,10)
        '''
        #scheduler = MC.config.create_scheduler(sess, 'tensorflow', 'TensorflowLevelParameterPruner',level)
        #scheduler.onEpochBegin(1)
        """@nni.compression.init(code_type='Tensorflow')"""
        """@nni.compression.on_epoch_begin()"""
        #on_epoch_begin(sess, 1)
        Compression().init('tensorflow')
        sess.run(tf.global_variables_initializer())
        #saver.restore(sess, 'model.ckpt')
        ...
        for i in range(params['batch_num']):
            #on_minibatch_begin(sess, i)
            Compression().apply_mask(sess, i)
            ...
            if i % 100 == 0:
                #scheduler.onMiniBatchBegin(1)
                Compression().apply_mask(sess, i)
                test_acc = mnist_network.accuracy.eval(
                    feed_dict={mnist_network.images: mnist.test.images,
                               mnist_network.labels: mnist.test.labels,
                               mnist_network.keep_prob: 1.0})
                print(i,'test accuracy %g', test_acc)
                logger.debug('test accuracy %g', test_acc)
                logger.debug('Pipe send intermediate result done.')
                for var in tf.all_variables():
                    if var.name == 'conv1/Variable:0':
                        num_elements = tf.size(var)
                        num_zero = tf.count_nonzero(var)
                        print('non_zero',sess.run([num_elements,num_zero]))
